                    ...
                    <div>
                        <table class="min-w-full border-collapse border border-gray-300 shadow-lg">
                            ...
                            <tbody class="bg-white">
                                ...
                                @elseif(isset($employees) && $employees->count() > 0)
                                    <!-- Display List of Employees -->
                                    @foreach($employees as $employee)
                                        <tr onclick="window.location='{{ route('employee.edit', $employee->id) }}'" 
                                            ...
                                            <td class="border px-4 py-2">
                                                <form action="{{ route('employee.destroy', $employee->id) }}" method="post" id="delete-form-{{ $employee->id }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" onclick="confirmDelete(event, {{ $employee->id }})" 
                                ...
                                @else
                                    <!-- No Records Found -->
                                    <tr>
                                        <td colspan="9" class="border px-4 py-2 text-center">No employee records found.</td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
